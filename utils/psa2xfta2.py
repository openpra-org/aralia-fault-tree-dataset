...
def transform_openpsa_to_xfta2(input_path, output_path):
    ...
    """
    try:
        # Registering the namespace can help with parsing, but for this
        # simple structure, direct tag access is sufficient.
    ...
    # As per the prompt's example (1-to-1 file), we process the first fault tree.
    # The spec allows for multiple, but the example implies a single output.
    ...
    for ccf_group in new_root.findall('.//define-CCF-group'):
        ...
        # Collect children to re-add them in the correct order
        children_to_reorder = {}
        if members is not None:
            children_to_reorder['members'] = members

        # Process factors: unwrap <factor> elements
        ...
        # Process distribution: unwrap <distribution> element
        if distribution is not None:
            # We assume distribution has one child element as per the example
            dist_child = next(iter(distribution), None)
            if dist_child is not None:
                 children_to_reorder['distribution_child'] = dist_child

        # Clear existing children and re-append in the specified order
        # We do this by removing all children and then adding them back
    ...
    # --- 5. Create and populate the new 'model-data' element ---
    ...
    # --- 6. Write the transformed XML to the output file ---
